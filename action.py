...
def _generate_manifest():
    return ["git", "ls-tree", "-r", "HEAD", "--name-only", "|", "xargs", "sha256sum", ">", "checksums"]
...
_debug("Generating repository checksums file.")

checksums_file_status = subprocess.run(
    _generate_manifest(),
    text=True,
...
_debug(checksums_file_status.stdout)

if checksums_file_status.returncode != 0:
    sigstore_sign_args = ["checksums"] + sigstore_sign_args
